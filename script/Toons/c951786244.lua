...
function s.initial_effect(c)
	c:EnableReviveLimit()
	c:EnableCounterPermit(0x11c2)
	--Synchro Summon
	Synchro.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsType,TYPE_TOON),1,1,Synchro.NonTuner(nil),1,99,s.matfilter)
	--splimit
...
function s.negcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsCanRemoveCounter(tp,0x11c2,1,REASON_COST) end
	e:GetHandler():RemoveCounter(tp,0x11c2,1,REASON_COST)
end
...
function s.spfilter(c,e,tp)
	return c:IsLevelBelow(9) and c:IsType(TYPE_TOON) and c:IsCanBeSpecialSummoned(e,0,tp,true,true)
end

...
function s.initial_effect(c)
	...
	--Negate an activated effect that targets another Toon card(s) you control
...
function s.negconfilter(c,tp)
	return c:IsFaceup() and c:IsType(TYPE_TOON) and c:IsControler(tp) and c:IsOnField()
end
...
function s.effcon(e,c,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function s.effcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.CheckReleaseGroupCost(tp,Card.IsType,1,false,nil,c,TYPE_TOON) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	local g=Duel.SelectReleaseGroupCost(tp,Card.IsType,1,1,false,nil,c,TYPE_TOON)

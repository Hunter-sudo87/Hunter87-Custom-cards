-- Toon Dinoruffia Kentregina
--
local s,id=GetID()
function s.initial_effect(c)
	c:EnableReviveLimit()
	-- 2 Toon monsters with different names
...
function s.ffilter(c,fc,sumtype,tp,sub,mg,sg)
	return c:IsType(TYPE_TOON,fc,sumtype,tp) and (not sg or not sg:IsExists(s.fusfilter,1,c,c:GetCode(fc,sumtype,tp),fc,sumtype,tp))
end
function s.fusfilter(c,code,fc,sumtype,tp)
	return c:IsSummonCode(fc,sumtype,tp,code) and not c:IsHasEffect(511002961)
end
function s.effilter(c)
	return c:IsAbleToRemoveAsCost() and c:IsSetCard(0x62) and c:GetType()==TYPE_TRAP

...
e5:SetCode(EVENT_PHASE+PHASE_BATTLE)
	--e5:SetCondition(function() return Duel.GetCurrentPhase()==PHASE_BATTLE end)
...
s.listed_names={80824380,84821955}
s.listed_series={0x131}
function s.cfilter(c,tp)
	return c:IsControler(tp) and c:IsPreviousControler(tp)
		and c:IsReason(REASON_DESTROY)
		and c:GetReasonEffect() and c:GetReasonEffect():GetOwner()==c and c:IsCode(80824380)
end
...
function s.filter(c,tp)
	return c:IsCode(84821955) and c:GetActivateEffect():IsActivatable(tp,true,true)
end
